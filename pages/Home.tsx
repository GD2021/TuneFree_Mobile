...
const SongCard = memo<{
  ...
  idx: number;
  onPlay: (s: Song) => void;
  onPrefetch: (s: Song) => void;
}>(({ song, idx, onPlay, onPrefetch }) => {
  ...
  return (
    ...
    >
      <span
        className={`font-bold text-lg w-6 text-center italic ${idx < 3 ? "text-ios-red" : "text-ios-subtext/50"}`}
      >
        {idx + 1}
      </span>
      ...
      <div className="flex-1 min-w-0">
        <p className="font-semibold text-ios-text truncate text-[15px]">
          {songName}
        </p>
        <div className="flex items-center mt-1 space-x-2">
          <span className="text-[10px] px-1 rounded bg-gray-100 text-gray-500 uppercase">
            {String(song.source)}
          </span>
...
const TopListSkeleton = () => (
  <div className="flex gap-3 overflow-hidden pb-2">
    {[0, 1, 2, 3].map((i) => (
      <div
        key={i}
        className="flex-shrink-0 bg-white p-2 rounded-2xl min-w-[120px] max-w-[140px] animate-pulse"
      >
...
const Home: React.FC = () => {
    ...
    try {
      ...
      if (lists && lists.length > 0) {
        ...
        try {
          ...
          _detailCache.set(`${source}:${lists[0].id}`, {
            songs: sliced,
            ts: Date.now(),
          });
        } catch (e) {
  ...
  const handleTopListClick = useCallback(
    async (list: TopList) => {
      // 检查缓存
  ...
  return (
    <div className="p-5 pt-safe min-h-screen bg-ios-bg">
      <div className="flex items-end justify-between mb-6 mt-2">
        <h1 className="text-3xl font-bold text-ios-text tracking-tight">
          {getGreeting()}
        </h1>
      </div>
            ...
            {topLists.map((list) => {
              ...
              return (
                ...
                >
                  <div className="w-full aspect-square mb-2 rounded-xl overflow-hidden bg-gray-100 relative">
                    {cover ? (
                      <img
                        src={cover}
                        alt={list.name}
                        referrerPolicy={getImgReferrerPolicy(cover)}
                        loading="lazy"
                        className="w-full h-full object-cover"
                      />
                    ) : (
                  ...
                  <p className="font-bold text-ios-text text-sm truncate px-1">
                    {String(list.name || "未知榜单")}
                  </p>
                  <p className="text-[10px] text-ios-subtext mt-0.5 truncate px-1">
                    {String(list.updateFrequency || "每日更新")}
                  </p>
                </button>
              );
            })}
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-ios-text tracking-tight">
            榜单热歌
          </h2>
          <span className="text-[10px] text-gray-400 bg-gray-100 px-2 py-1 rounded-full uppercase">
            {activeSource}
          </span>
        </div>
        ...
        {songsLoading && featuredSongs.length === 0 ? (
          ...
          <div className="pb-24">
            <List
              ...
              width="100%"
              itemKey={(index) =>
                `${featuredSongs[index].source}-${featuredSongs[index].id}`
              }
            >
