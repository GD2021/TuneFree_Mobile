...
const PlayerMorePopup: React.FC<PlayerMorePopupProps> = ({
  isOpen,
  onClose,
  onClosePlayer,
}) => {
  const { currentSong, audioQuality } = usePlayerState();
  const { setAudioQuality } = usePlayerActions();
  const { playlists, addToPlaylist, createPlaylist } = useLibraryData();
        ...
        {!showPlaylistSelect ? (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              <button
                onClick={() =>
                  currentSong.artist && handleSearch(currentSong.artist)
                }
              ...
              <button
                onClick={() =>
                  currentSong.album && handleSearch(currentSong.album)
                }
