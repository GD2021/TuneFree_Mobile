      ...
      if (isPlaying && analyser) {
        ...
        for (let i = 0; i < BAR_COUNT; i++) {
          const startBin = Math.max(
            1,
            Math.round(Math.exp((logMax * i) / BAR_COUNT)),
          );
          const endBin = Math.max(
            startBin + 1,
            Math.round(Math.exp((logMax * (i + 1)) / BAR_COUNT)),
          );
          ...
          if (percent > state.displayValues[i]) {
            state.displayValues[i] +=
              (percent - state.displayValues[i]) * (1 - SMOOTHING_ALPHA);
          } else {
        ...
        for (let i = 0; i < BAR_COUNT; i++) {
          ...
          const noise =
            (Math.sin(i * 0.3 + state.phase) +
              Math.sin(i * 0.7 - state.phase)) *
            20;
